...
- **反思与人工参与**：负责人明确要求 Codex 根据其全过程中的真实选择、逐项审批、审阅意见和项目证据起草 `REFLECTION.md`；Codex 同时辅助整理、校对与润色。负责人未提供人工初稿，需在课程提交前亲自通读、核对并按本人理解修改。PR-05 生产代码与自动测试由 `codex-main` 实现，负责人提供最终范围授权。
- **最终本地门禁**：直接执行仓库 PowerShell 脚本先被系统 ExecutionPolicy 阻止；使用一次性 Bypass 后 pytest 又因系统默认临时目录 ACL 出现 `WinError 5`，并暴露 README/反思固定短语的 1 个真实失败。改用显式可写 basetemp 后得到 `392 passed, 3 skipped, 1 failed`，补齐“Codex 辅助整理、校对与润色”的披露短语后，fresh 全量为 `393 passed, 3 skipped`。Ruff 全通过，strict mypy 对 23 个源码文件通过，wheel/sdist 构建成功；3 个 skip 仍仅为 Windows `WinError 1314` 符号链接权限。
- **反思口径更正**：PR-05 尚未推送的早期 commit body 和文档曾将反思来源误记为“负责人提供人工初稿”。负责人本轮实际指令是由 Codex 直接根据项目过程起草，因此本条及当前 `REFLECTION.md` 更正并覆盖早期措辞；不通过重写审计历史掩盖该更正。
...
- **授权与工作流**：负责人明确授权整个仓库、`REFLECTION.md`、Git 整理、commit 与 push，并要求本次不新建分支或 PR、直接在 `main` 完成。该要求是四个课程 PR 全部合并后的 post-delivery 修订例外；本条如实记录，不把本次直推描述成新的 PR 工作流。人工代码修改：无；反思报告由 Codex 根据负责人真实决策与项目证据起草，并披露 AI 辅助。
...
- **文档与学术规范**：README 已提供真实/离线运行、两级 TOML、CLI 收紧参数、协议限制和直接依赖许可证清单。`REFLECTION.md` 由 Codex 根据负责人真实选择、逐项审批和项目证据起草并明确披露，负责人需在课程提交前亲自核对和修改；正文处于 1500–2500 字范围，没有虚构真实 LLM 或 Claude Code 实验。PLAN Task 6–14 的历史空复选框已按既有实施证据机械标记完成。
